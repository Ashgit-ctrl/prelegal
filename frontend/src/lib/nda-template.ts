...
<ol>
  ...
  <li><strong>Term and Termination.</strong> This MNDA commences on the <em class="coverpage-value effective-date-value"></em> and expires at the end of the <em class="coverpage-value mnda-term-value"></em>. Either party may terminate this MNDA for any or no reason upon written notice to the other party. The Receiving Party's obligations relating to Confidential Information will survive for the <em class="coverpage-value confidentiality-term-value"></em>, despite any expiration or termination of this MNDA.</li>
  ...
  <li><strong>Governing Law and Jurisdiction.</strong> This MNDA and all matters relating hereto are governed by, and construed in accordance with, the laws of the State of <em class="coverpage-value governing-law-value"></em>, without regard to the conflict of laws provisions of such <em class="coverpage-value governing-law-value"></em>. Any legal suit, action, or proceeding relating to this MNDA must be instituted in the federal or state courts located in <em class="coverpage-value jurisdiction-value"></em>. Each party irrevocably submits to the exclusive jurisdiction of such <em class="coverpage-value jurisdiction-value"></em> in any such suit, action, or proceeding.</li>
...
export function getMndaTermText(data: NDAFormData): string {
  if (data.mndaTermType === "expires") {
    const years = data.mndaTermYears;
    return `${years} ${years === 1 ? "year" : "years"} from Effective Date`;
  }
  return "the date of termination in accordance with the terms of the MNDA";
}
...
export function getConfidentialityTermText(data: NDAFormData): string {
  if (data.confidentialityTermType === "years") {
    ...
    return `${years} ${years === 1 ? "year" : "years"} from Effective Date (but in the case of trade secrets, until the Confidential Information is no longer a trade secret under applicable law)`;
  }
  return "perpetuity";
}
